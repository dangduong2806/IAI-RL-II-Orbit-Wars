...
MLP_MODEL = None
_MLP_LOAD_FAILED = False
def get_mlp_model(device):
    global MLP_MODEL, _MLP_LOAD_FAILED
    if _MLP_LOAD_FAILED:
        return None
    if MLP_MODEL is None:
        try:
            import os
            MLP_MODEL = MLPCommander().to(device)
            current_dir = os.path.dirname(os.path.abspath(__file__))
            model_path = os.path.join(current_dir, "best_deep.pth")
            MLP_MODEL.load_state_dict(torch.load(model_path, map_location=device))
            MLP_MODEL.eval()
        except Exception:
            MLP_MODEL = None
            _MLP_LOAD_FAILED = True
            return None
    return MLP_MODEL

def _compute_mlp_score_for_candidates(status: GarrisonPrediction, prod: Tensor, alive_by_step: Tensor, player_count: int, launches, player_id: int) -> Tensor | None:
    """Compute per-candidate MLP score. Returns (num_candidates,) or None on failure."""
    try:
        device = prod.device
        ...
        src_all = launches.source_slots   # (C, L)
        tgt_all = launches.target_slots   # (C, L)
        ships_all = launches.ships        # (C, L)
        eta_all = launches.eta            # (C, L)
        valid_all = launches.valid        # (C, L)

        num_candidates = src_all.shape[0]
        if num_candidates == 0:
            return None

        src = src_all[:, 0].clamp(0, status.ships.shape[0] - 1)
        tgt = tgt_all[:, 0].clamp(0, status.ships.shape[0] - 1)
        send = ships_all.sum(dim=-1)  # total ships across all sub-launches
        eta = eta_all[:, 0]

        num_planets = status.ships.shape[0]
        owner0 = status.owner[:, 0]
        ships0 = status.ships[:, 0].float()
        alive0 = alive_by_step[0]

        mine_mask = (owner0 == player_id) & alive0
        total_alive = alive0.sum().float().clamp(min=1.0)
        my_cnt = mine_mask.sum().float()
        my_prod_total = prod[mine_mask].sum()
        total_prod = prod[alive0].sum().clamp(min=1e-6)
        my_ships_total = ships0[mine_mask].sum()
        total_ships = ships0[alive0].sum().clamp(min=1e-6)

        out = torch.zeros((num_candidates, 46), dtype=torch.float32, device=device)

        out[:, 0] = 0.5
        out[:, 1] = float(player_count) / 4.0
        out[:, 2] = (my_cnt / total_alive).clamp(0, 1)
        out[:, 3] = (my_prod_total / total_prod).clamp(0, 1)
        out[:, 4] = (my_ships_total / total_ships).clamp(0, 1)

        def _ssig(x):
            return x.clamp(min=0.0) / (x.clamp(min=0.0) + 40.0)

        src_ships = ships0[src]
        tgt_owner = owner0[tgt]
        out[:, 12] = _ssig(src_ships)
        out[:, 13] = (prod[src] / 5.0).clamp(0, 1)
        out[:, 14] = 1.0
        out[:, 15] = torch.where(tgt_owner == player_id, 1.0, torch.where(tgt_owner < 0, 0.0, -1.0))
        out[:, 16] = (prod[tgt] / 5.0).clamp(0, 1)
        out[:, 18] = (ships0[tgt] / total_ships).clamp(0, 1)

        H = status.owner.shape[1] - 1
        eta_idx = eta.clamp(1, H).long()
        proj_owner = status.owner[tgt, eta_idx]
        proj_ships = status.ships[tgt, eta_idx].float()
        out[:, 20] = torch.where(proj_owner == player_id, 1.0, torch.where(proj_owner < 0, 0.0, -1.0))
        out[:, 21] = _ssig(proj_ships)
        out[:, 27] = (eta / 25.0).clamp(0, 1)
        out[:, 28] = (send / (my_ships_total + 1e-6)).clamp(0, 1)
        need = torch.where(proj_owner == player_id, torch.zeros_like(proj_ships), proj_ships.ceil() + 1.0)
        out[:, 29] = (need / (src_ships + 1e-6)).clamp(0, 2)
        margin = send - need
        out[:, 30] = (margin / (send + 40.0)).clamp(-1, 1)
        out[:, 31] = (send > need).float()
        survivors = torch.where(tgt_owner == player_id, send, (send - proj_ships).clamp(min=0.0))
        garrison = _ssig(survivors)
        out[:, 32] = garrison
        out[:, 33] = garrison
        out[:, 34] = (prod[tgt] / (need + 1.0)).clamp(0, 1)
        out[:, 42] = 1.0
        out[:, 43] = 0.5
        out[:, 44] = (prod[tgt] / (my_prod_total + 1.0)).clamp(0, 1)

        out = torch.nan_to_num(out, 0.0)

        with torch.no_grad():
            mlp_score = mlp(out).squeeze(-1)  # (num_candidates,)
        return mlp_score
    except Exception:
        return None

MLP_BLEND_WEIGHT = 1.0
...
def evaluate_launch_options(status: GarrisonPrediction, *, prod: Tensor, alive_by_step: Tensor, player_count: int, launches: FleetLaunchGroup, player_id: int) -> Tensor:
    ...
    mlp_score = _compute_mlp_score_for_candidates(
        status, prod, alive_by_step, player_count, launches, player_id
    )

    if mlp_score is not None:
        return mlp_score

    diff = compute_hypothetical_flow_delta(
        status, prod=prod, alive_by_step=alive_by_step,
        player_count=int(player_count), launches=launches, player_id=int(player_id)
    )
