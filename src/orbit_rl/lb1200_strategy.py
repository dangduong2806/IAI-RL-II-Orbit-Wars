...
try:
    from submission import feature46_weights_2p as feature46_scorer_2p
    from submission import feature46_weights_4p as feature46_scorer_4p
except Exception:
    feature46_scorer_2p = None
    feature46_scorer_4p = None
...
FEATURE46_ENABLED = True
FEATURE46_K_SHIPS = 40.0
FEATURE46_EPS = 1.0e-6
FEATURE46_LAMBDA_ECEP = 0.3
FEATURE46_LAMBDA_POTENTIAL = 0.15
FEATURE46_HISTORY_SIZE = 5
FEATURE46_RHYTHM_SIZE = 10
FEATURE46_MAX_ETA = 25.0
FEATURE46_SCORE_MULT_2P = 0.30
FEATURE46_SCORE_MULT_4P = 0.22
...
def _f46_clamp(x, lo=0.0, hi=1.0):
    return max(lo, min(hi, float(x)))


def _f46_ship_sigmoid(x):
    x = max(0.0, float(x))
    return x / (x + FEATURE46_K_SHIPS)
...
def _f46_direct_eta_obj(a, b, ships):
    d = planet_distance(a, b)
    return max(1, int(math.ceil(d / fleet_speed(max(1, int(ships)))))), d


def _f46_direct_eta_xy(ax, ay, bx, by, ships):
    d = dist(ax, ay, bx, by)
    return max(1, int(math.ceil(d / fleet_speed(max(1, int(ships)))))), d


def _f46_center_of_mass(world, owner):
    sx = sy = sw = 0.0
    for planet in world.planets:
        if int(planet.owner) != int(owner):
            continue
        w = max(1.0, float(planet.ships))
        sx += float(planet.x) * w
        sy += float(planet.y) * w
        sw += w
    if sw <= 0.0:
        for fleet in world.fleets:
            if int(fleet.owner) != int(owner):
                continue
            w = max(1.0, float(fleet.ships))
            sx += float(fleet.x) * w
            sy += float(fleet.y) * w
            sw += w
    if sw <= 0.0:
...
def _f46_angular_spread(my_com, enemy_coms):
    if len(enemy_coms) <= 1:
        return 0.0
    angles = sorted(math.atan2(y - my_com[1], x - my_com[0]) for x, y in enemy_coms)
    gaps = [angles[i + 1] - angles[i] for i in range(len(angles) - 1)]
    gaps.append(angles[0] + 2.0 * math.pi - angles[-1])
    return _f46_clamp(1.0 - max(gaps) / (2.0 * math.pi))


class _Feature46History:
    def __init__(self):
        self.ship_share = {i: deque(maxlen=FEATURE46_HISTORY_SIZE) for i in range(4)}
        self.targeted = deque(maxlen=FEATURE46_HISTORY_SIZE)
        self.incoming = deque(maxlen=FEATURE46_RHYTHM_SIZE)
    ...
    def update(self, step, player, shares, targeted_ratio, hostile_incoming, enemy_dists, convergence_raw):
        ...
        momentum = float(shares.get(player, 0.0)) - float(old_my[0]) if len(old_my) >= FEATURE46_HISTORY_SIZE else 0.0
        enemy_momentums = []
        for owner in range(4):
            old = self.ship_share.get(owner, deque())
            if owner == player or len(old) < FEATURE46_HISTORY_SIZE:
                continue
        ...
        aggression_trend = (
            float(targeted_ratio) - float(self.targeted[0])
            if len(self.targeted) >= FEATURE46_HISTORY_SIZE
            else 0.0
        )

        enemy_rhythm = 0.0
        if len(self.incoming) >= FEATURE46_RHYTHM_SIZE:
            vals = [float(v) for v in self.incoming]
            mean = sum(vals) / max(1, len(vals))
            var = sum((v - mean) ** 2 for v in vals) / max(1, len(vals))
            enemy_rhythm = _f46_clamp(var / (mean * mean + FEATURE46_EPS))
        ...
        if self.prev_enemy_dists:
            ...
            for owner, cur in enemy_dists.items():
                ...
                n += 1
            approach_rate = _f46_clamp(total / (max(1, n) * 12.0 + FEATURE46_EPS))

        convergence = _f46_clamp(0.7 * self.convergence_prev + 0.3 * float(convergence_raw))
        self.convergence_prev = convergence
        for owner in range(4):
            self.ship_share.setdefault(owner, deque(maxlen=FEATURE46_HISTORY_SIZE)).append(float(shares.get(owner, 0.0)))
        ...
        self.last_step = step
        self.last_outputs = {
            "momentum": _f46_clamp(momentum, -1.0, 1.0),
            "fastest_grower_gap": _f46_clamp(fastest_gap, -1.0, 1.0),
            "aggression_trend": _f46_clamp(aggression_trend, -1.0, 1.0),
...
_FEATURE46_HISTORY_BY_PLAYER = {}
_FEATURE46_CACHE_BY_PLAYER = {}


def _f46_history_for(world):
    hist = _FEATURE46_HISTORY_BY_PLAYER.get(int(world.player))
    if hist is None:
        hist = _Feature46History()
        _FEATURE46_HISTORY_BY_PLAYER[int(world.player)] = hist
    return hist


def _f46_nearest_enemy_distance(world, planet):
    if not world.enemy_planets:
        return 1.0e9
    return min(dist(planet.x, planet.y, enemy.x, enemy.y) for enemy in world.enemy_planets)


def _f46_feature_cache(world):
    player = int(world.player)
    cached = _FEATURE46_CACHE_BY_PLAYER.get(player)
    ...
    for planet in world.planets:
        if int(planet.owner) == -1:
            continue
        rec = totals[int(planet.owner)]
        rec["planets"] += 1
        rec["planet_ships"] += float(planet.ships)
        rec["prod"] += float(planet.production)
        alive.add(int(planet.owner))
    for fleet in world.fleets:
        rec = totals[int(fleet.owner)]
        rec["fleet_ships"] += float(fleet.ships)
        rec["transit"] += float(fleet.ships)
        alive.add(int(fleet.owner))
    ...
    enemy_owners = [owner for owner in totals if owner not in (-1, player) and totals[owner].get("total_ships", 0.0) > 0.0]
    max_enemy_ships = max((totals[owner].get("total_ships", 0.0) for owner in enemy_owners), default=0.0)

    hostile_transit = sum(float(fleet.ships) for fleet in world.fleets if int(fleet.owner) not in (-1, player))
    hostile_to_me = 0.0
    my_planet_ids = {int(planet.id) for planet in world.my_planets}
    ...
    targeted_ratio = hostile_to_me / (hostile_transit + FEATURE46_EPS)

    my_com = _f46_center_of_mass(world, player)
    enemy_com = {owner: _f46_center_of_mass(world, owner) for owner in enemy_owners}
    enemy_dists = {owner: dist(my_com[0], my_com[1], com[0], com[1]) for owner, com in enemy_com.items()}
    ...
    convergence_raw = _f46_clamp(min_enemy_dist / (avg_enemy_dist_raw + FEATURE46_EPS)) if enemy_dists else 0.0
    shares = {owner: rec.get("total_ships", 0.0) / (total_ships + FEATURE46_EPS) for owner, rec in totals.items()}
    hist = _f46_history_for(world).update(int(world.step), player, shares, targeted_ratio, hostile_to_me, enemy_dists, convergence_raw)

    weakest_enemy = min(enemy_owners, key=lambda owner: totals[owner].get("total_ships", 0.0)) if enemy_owners else None
    weakest_colony = 1.0
    if world.my_planets:
        for planet in world.my_planets:
            hostile = sum(
                ...
                if int(owner) not in (-1, player) and int(eta) <= FEATURE46_MAX_ETA
            )
            weakest_colony = min(weakest_colony, _f46_clamp((float(planet.ships) - hostile) / (float(planet.ships) + FEATURE46_EPS)))
    else:
        weakest_colony = 0.0

    macro = {
        "game_progress": _f46_clamp(float(world.step) / float(TOTAL_STEPS)),
        "players_alive": _f46_clamp(len(alive) / 4.0),
        "my_planet_share": _f46_clamp(float(len(world.my_planets)) / max(1.0, float(len(world.planets)))),
        "my_gdp_share": _f46_clamp(my_prod / (total_prod + FEATURE46_EPS)),
        "my_ship_share": _f46_clamp(my_total / (total_ships + FEATURE46_EPS)),
        "momentum": hist["momentum"],
        "my_fleet_ratio": _f46_clamp(float(my.get("transit", 0.0)) / (my_total + FEATURE46_EPS)),
        "leader_gap": _f46_clamp((max_enemy_ships - my_total) / (my_total + FEATURE46_K_SHIPS), -1.0, 2.0),
        "am_i_targeted": _f46_clamp(targeted_ratio),
        "fastest_grower_gap": hist["fastest_grower_gap"],
        "aggression_trend": hist["aggression_trend"],
        "enemy_rhythm": hist["enemy_rhythm"],
        "avg_enemy_distance": _f46_clamp(avg_enemy_dist_raw / (math.sqrt(2.0) * BOARD)),
        "angular_spread": _f46_angular_spread(my_com, list(enemy_com.values())),
    ...
    _FEATURE46_CACHE_BY_PLAYER[player] = cached
    return cached


def _f46_candidate_features(world, src, target):
    cache = _f46_feature_cache(world)
    player = int(world.player)
    src_ships = max(1, int(src.ships))
    send = src_ships
    aim = world.plan_shot(src.id, target.id, send)
    if aim is None:
        eta = max(1, int(math.ceil(planet_distance(src, target) / fleet_speed(send))))
    else:
        _angle, eta, _dist_to_target, _path_target = aim

    proj_owner, proj_ships = world.projected_state(target.id, eta)
    ...
    owner_power = _f46_clamp(target_total / (cache["total_ships"] + FEATURE46_EPS))
    weakest_enemy = cache["weakest_enemy"]

    src_threat = sum(
        _f46_ship_sigmoid(ships) * math.exp(-FEATURE46_LAMBDA_ECEP * float(eta0))
        for eta0, owner, ships in world.arrivals_by_planet.get(src.id, [])
        if int(owner) not in (-1, player)
    )
    src_safety = _f46_clamp(1.0 - src_threat / (src_threat + 1.0))
    arrivals = world.arrivals_by_planet.get(target.id, [])
    threat_after = sum(
        _f46_ship_sigmoid(ships) * math.exp(-FEATURE46_LAMBDA_ECEP * max(0.0, float(eta0) - float(eta)))
    ...
    pre_volatility = sum(
        _f46_ship_sigmoid(ships) * math.exp(-FEATURE46_LAMBDA_ECEP * max(0.0, float(eta) - float(eta0)))
        for eta0, owner, ships in arrivals
        if int(eta0) < int(eta)
    )
    threat_potential_raw = 0.0
    for enemy in world.enemy_planets:
        e_eta, _ = _f46_direct_eta_obj(enemy, target, max(1, int(enemy.ships)))
        threat_potential_raw += _f46_ship_sigmoid(enemy.ships) * math.exp(-FEATURE46_LAMBDA_POTENTIAL * e_eta)
    support_potential_raw = 0.0
    for ally in world.my_planets:
        if int(ally.id) == int(src.id):
            continue
        s_eta, _ = _f46_direct_eta_obj(ally, target, max(1, int(ally.ships)))
        support_potential_raw += _f46_ship_sigmoid(ally.ships) * math.exp(-FEATURE46_LAMBDA_POTENTIAL * s_eta)
    threat_potential = _f46_clamp(min(threat_potential_raw, 2.0) / 2.0)
    support_potential = _f46_clamp(min(support_potential_raw, 2.0) / 2.0)
    ...
    garrison_strength = _f46_ship_sigmoid(survivors)
    defense_sustainability = _f46_clamp(garrison_strength / (float(threat_after) + FEATURE46_EPS))
    src_front = _f46_nearest_enemy_distance(world, src)
    tgt_front = _f46_nearest_enemy_distance(world, target)
    src_x2, src_y2 = predict_planet_position(src, world.initial_by_id, world.ang_vel, 1)
    target_next = predict_target_position(target, 1, world.initial_by_id, world.ang_vel, world.comets, world.comet_ids)
    if target_next is None:
        target_next = (target.x, target.y)
    eta_next, _ = _f46_direct_eta_xy(src_x2, src_y2, target_next[0], target_next[1], send)
    orbital_trend = _f46_clamp((float(eta_next) - float(eta)) / (float(eta) + FEATURE46_EPS), -1.0, 1.0)
    enemy_commitment = 0.0
    if target_owner not in (-1, player):
        rec = cache["totals"].get(target_owner, {})
        enemy_commitment = _f46_clamp(float(rec.get("transit", 0.0)) / (float(rec.get("total_ships", 0.0)) + FEATURE46_EPS))
    local_superiority = _f46_clamp(min(support_potential / (threat_potential + 0.1), 3.0) / 3.0)
    economic_impact = _f46_clamp(float(target.production) / (cache["my_prod"] + 1.0))
    enemy_exposed = _f46_clamp(enemy_commitment * (1.0 - owner_power))
    macro = cache["macro"]
    row = [
        macro["game_progress"],
        macro["players_alive"],
        ...
        macro["enemy_rhythm"],
        _f46_ship_sigmoid(src_ships),
        _f46_clamp(float(src.production) / 5.0),
        src_safety,
        diplomacy,
        _f46_clamp(float(target.production) / 5.0),
        1.0 if int(target.id) in world.comet_ids or not is_static_planet(target) else 0.0,
        owner_power,
        ...
        _f46_ship_sigmoid(proj_ships),
        _f46_clamp(threat_after),
        _f46_clamp(support_after),
        _f46_clamp(pre_volatility),
        threat_potential,
        support_potential,
        _f46_clamp(float(eta) / FEATURE46_MAX_ETA),
        _f46_clamp(float(send) / (cache["my_total"] + FEATURE46_EPS)),
        _f46_clamp(need / (float(src_ships) + FEATURE46_EPS), 0.0, 2.0),
        _f46_clamp(margin / (float(send) + FEATURE46_K_SHIPS), -1.0, 1.0),
        1.0 if float(send) > need else 0.0,
        garrison_strength,
        defense_sustainability,
        _f46_clamp(float(target.production) / (need + 1.0)),
        _f46_clamp((src_front - tgt_front) / 60.0, -1.0, 1.0),
...
def _feature46_scorer(world):
    if not FEATURE46_ENABLED:
        return None, 0.0
    if int(getattr(world, "num_players", 2)) <= 2:
        return feature46_scorer_2p, FEATURE46_SCORE_MULT_2P
    return feature46_scorer_4p, FEATURE46_SCORE_MULT_4P


def apply_feature46_score(base_score, src, target, world):
    if base_score <= 0.0 or src is None or target is None:
        return base_score
    scorer, strength = _feature46_scorer(world)
    if scorer is None or strength <= 0.0:
        return base_score
    try:
        row = _f46_candidate_features(world, src, target)
        score = _f46_sigmoid(float(scorer.score_many([row])[0]))
    except Exception:
        return base_score
    mult = 1.0 + strength * (score - 0.5)
...
def build_snipe_mission(src, target, src_available, world, planned_commitments, modes, policy):
    ...
    for enemy_eta in enemy_etas[:3]:
        ...
        score = apply_feature46_score(score, src, target, world)
        option = ShotOption(
            score=score,
            src_id=src.id,
            target_id=target.id,
...
def plan_moves(world, deadline=None, mission_selector=None, top_k=None):
    def expired():
        return deadline is not None and time.perf_counter() > deadline
                ...
                if partial_seed is not None:
                    ...
                    if time_filters_pass(target, p_turns, global_needed, src_available):
                        ...
                        if partial_value > 0:
                            ...
                            partial_score = apply_feature46_score(partial_score, src, target, world)
    ...
    for target_id, options in source_options_by_target.items():
        ...
        for i in range(len(top_options)):
            for j in range(i + 1, len(top_options)):
                ...
                pair_score = apply_feature46_score(pair_score, world.planet_by_id.get(first.src_id), target, world)
            ...
            for i in range(len(top_options)):
                for j in range(i + 1, len(top_options)):
                    for k in range(j + 1, len(top_options)):
                        ...
                        trio_score = apply_feature46_score(trio_score, world.planet_by_id.get(first.src_id), target, world)
    ...
    # ---------------------------------------------------------------
    # RL hook: let an external policy select/reorder/filter top-K missions.
    # If mission_selector is None, the original lb-1200 heuristic behavior
    # is preserved. The selector must return a list of Mission objects.
    # ---------------------------------------------------------------
    if mission_selector is not None:
        try:
            selected_missions = mission_selector(world=world, missions=missions, top_k=top_k)
        except TypeError:
            selected_missions = mission_selector(world, missions, top_k)
        if selected_missions is not None:
            missions = list(selected_missions)
    elif top_k is not None:
        missions = missions[: int(top_k)]
    ...
    for mission in missions:
        ...
        swarm_margin = 0
        if target.owner == -1:
            swarm_margin = min(NEUTRAL_MARGIN_CAP, SAFE_NEUTRAL_MARGIN + int(target.production))
        elif target.owner != world.player:
            swarm_margin = min(HOSTILE_MARGIN_CAP, HOSTILE_MARGIN_BASE + int(target.production))
        if world.is_four_player:
            swarm_margin += FOUR_PLAYER_TARGET_MARGIN
        desired_total = min(sum(limits), missing + swarm_margin)
    ...
    if live_doomed:
        ...
        for planet in world.my_planets:
            ...
            for target in world.planets:
                ...
                score = apply_feature46_score(score, planet, target, world)
                if target.owner not in (-1, world.player):
                    score *= 1.05
...
__all__ = ["agent", "build_world", "plan_moves", "WorldModel", "Mission", "ShotOption", "Planet", "Fleet"]
